...
import com.qdd.mongodb.demo.exception.JsonException;
...
public class BaseResultGenerator {
     ...
     * @return 操作成功的默认响应结果
     */
    public static <T> BaseResult<T> success(final int code, final String message) {
    	return new BaseResult<>(code, true, message, null);
    }
    ...
    public static <T> BaseResult<T> error(final JsonException be) {
        return new BaseResult<>(BaseResultEnum.SERVER_ERROR.getCode(), false, be.getMessage(), null);
    }
